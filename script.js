...
const renderDatalab = (payload) => {
  ...
  const priorityCategory =
    categories.find((category) => category.name === recommendation.priorityCategory) ||
    categories[0];
      ...
      return `
        <div class="score-row">
          <div class="score-head">
            <strong>${item.brand || "브랜드명 없음"}</strong>
  ...
  datalabResults.innerHTML = `
    <div class="result-item">
      <span class="result-label">공략 카테고리</span>
      <strong>${recommendation.priorityMessage || "추천 데이터를 준비 중입니다."}</strong>
    </div>
    <div class="result-item">
      <span class="result-label">점유율 전환 전략</span>
      <strong>${recommendation.regionalMessage || "점유율 전환 전략 데이터를 준비 중입니다."}</strong>
    </div>
    <div class="result-item">
      <span class="result-label">채널 실행 전략</span>
      <strong>${recommendation.promoMessage || "채널 실행 전략 데이터를 준비 중입니다."}</strong>
    </div>
  `;
...
fetch(`./assets/data/datalab.json?v=${Date.now()}`, { cache: "no-store" })
  .then((response) => {
    if (!response.ok) {
  ...
  .catch(() => {
    renderDatalab({
      ...
      recommendation: {
        ...
        priorityMessage:
          "데이터 파일을 불러오지 못했습니다. GitHub Actions 연동 후 깨끗한나라가 우선적으로 공략할 카테고리와 영업 우선순위가 자동 갱신됩니다.",
        regionalMessage:
          "경쟁 브랜드가 강한 상품군에서는 비교 진열, 묶음 구성, 핵심 USP 메시지를 결합해 고객 선택을 깨끗한나라로 전환하는 전략으로 확장할 수 있습니다.",
        promoMessage:
          "대리점과 농협 채널에서 시즌성 높은 SKU 전면 배치, 입수량 행사, 생활밀착형 판촉 메시지를 연결한 실행 전략으로 활용하도록 설계했습니다.",
      },
    });
  });
