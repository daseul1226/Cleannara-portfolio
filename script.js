...
const datalabKeywordPills = document.getElementById("datalab-keyword-pills");
...
const renderDatalab = (payload) => {
  if (!datalabStatus || !datalabKeywordPills || !datalabScoreboard || !datalabResults) {
    return;
  }
  ...
  datalabKeywordPills.innerHTML = products
    .map((item) => `<span>${item.name}</span>`)
    .join("");

  const maxScore = Math.max(...products.map((item) => Number(item.score) || 0), 1);
  datalabScoreboard.innerHTML = products
    .map((item) => {
...
fetch("./assets/data/datalab.json")
  ...
  .catch(() => {
    renderDatalab({
      generatedAt: null,
      products: [
        { name: "물티슈", score: 0, last3Avg: 0, momentum: 0 },
        { name: "화장지", score: 0, last3Avg: 0, momentum: 0 },
        { name: "키친타월", score: 0, last3Avg: 0, momentum: 0 },
      ],
      recommendation: {
        priorityMessage: "데이터 파일을 불러오지 못했습니다. GitHub Actions 연동 후 자동 갱신됩니다.",
        regionalMessage: "지역 추천은 내부 판매 데이터와 결합 시 확장됩니다.",
        promoMessage: "검색량과 시즌성을 기준으로 프로모션 타이밍을 제안하도록 설계했습니다.",
      },
    });
  });
